...
if [ "$exclusive_dfg_count" -neq 11 ]; then
    ...
    exit 1
# else if [ "$inclusive_dfg_count" -neq 11 ]; then
#     echo "Multi-Cycle Test Failed! The count of DFG nodes in the inclusive strategy shoule be 11, but got $inclusive_dfg_count."
#     exit 1
else if [ "$distributed_dfg_count" -neq 16 ]; then
    echo "Multi-Cycle Test Pass! The count of DFG nodes in the distributed strategy shoule be 16, but got $distributed_dfg_count."
    exit 1
else
